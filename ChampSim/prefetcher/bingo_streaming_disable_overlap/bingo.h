...
public:
    ...
    std::vector<int> find_streaming(uint64_t trigger, uint64_t second, uint64_t pc)
    {
        ...
        int upper_bound = strong_stream ? this->pattern_len : std::max(this->pattern_len / 4, 2);
    ...
    int prefetch(CACHE* cache, uint64_t block_num)
    {
        ...
        for (int d = 1; d < this->pattern_len; d += 1) {
            for (int sgn = +1; sgn >= -1; sgn -= 2) {
                ...
                // This variant now treats every issued Bingo prefetch as an L2-targeted fill.
                pf_metadata = __add_pf_dest_level(pf_metadata, 2);

                cache->prefetch_line(0, base_addr, pf_address, true, pf_metadata);
                pf_issued += 1;
                pattern[pf_offset] = 0;
            }
        }
