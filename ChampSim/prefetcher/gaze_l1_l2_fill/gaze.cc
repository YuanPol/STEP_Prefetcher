...
static std::vector<gaze_l1::Gaze_L1> prefetchers;
...
void PrefetchBuffer::insert(uint64_t region_num, std::vector<int> pattern, uint64_t trigger, uint64_t second, uint32_t pf_metadata) {
    ...
    if ((pf_metadata & 3) == 0 || (pf_metadata & 3) == 3) { // stride & promote
        auto entry = find(key);
        if (!entry) {
            Super::insert(key, {pattern, trigger, trigger, std::vector<int>(gaze_l1::NUM_BLOCKS, pf_metadata)});
            Super::rp_insert(key);
        } else {
        ...
        Super::insert(key, {pattern, trigger, second, std::vector<int>(gaze_l1::NUM_BLOCKS, pf_metadata)});
        Super::rp_insert(key);
    }
}
...
Gaze_L1::Gaze_L1(int ft_size, int ft_ways, int at_size, int at_ways, int pt_size, int pt_ways, int pb_size, int pb_ways, int cpu = 0) :
    ft(ft_size, ft_ways), at(at_size, at_ways), pt(pt_size, pt_ways), pb(pb_size, pb_ways), cpu(cpu) {
}

void Gaze_L1::access(uint64_t block_num, uint64_t pc, CACHE* cache) {
    uint64_t region_num = block_num >> (LOG2_REGION_SIZE - LOG2_BLOCK_SIZE);
    uint64_t region_offset = __region_offset(block_num);
    ...
    if (at_entry) {
        ...
        if (!entry) {
            ...
            cache->sim_stats.total_trigger++;
            if (pt_entry){
                cache->sim_stats.pht_trigger++;
            }
            if (!pattern_empty) {
                // cache->sim_stats.pht_trigger++;
...
void Gaze_L1::eviction(uint64_t block_num) {
    uint64_t region_num = block_num >> (LOG2_REGION_SIZE - LOG2_BLOCK_SIZE);
    ft.erase(region_num);
...
void Gaze_L1::prefetch(CACHE* cache, uint64_t block_num) {
...
void Gaze_L1::tune_stride_degree(CACHE* cache) {}

PatternTable::Entry* Gaze_L1::find_in_pt(uint64_t trigger, uint64_t second, uint64_t pc, uint64_t region_num) {
...
void Gaze_L1::insert_in_pt(const AccumulateTable::Entry& entry, uint64_t region_num) {
...
void Gaze_L1::set_warmup(bool warmup) {
    this->warmup = warmup;
...
void CACHE::prefetcher_initialize() {
    std::cout << NAME << " Gaze_L1 NEW NEW prefetcher" << std::endl;

    prefetchers = std::vector<gaze_l1::Gaze_L1>(NUM_CPUS, gaze_l1::Gaze_L1(gaze_l1::FT_SIZE, gaze_l1::FT_WAY, gaze_l1::AT_SIZE, gaze_l1::AT_WAY, gaze_l1::PT_SIZE, gaze_l1::PT_WAY, gaze_l1::PB_SIZE, gaze_l1::PB_WAY, cpu));
}

uint32_t CACHE::prefetcher_cache_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type, uint32_t metadata_in) {
    uint64_t line_addr = (addr >> LOG2_BLOCK_SIZE); 
    uint64_t region_num = (addr >> LOG2_PAGE_SIZE);
    int offset = line_addr % gaze_l1::NUM_BLOCKS;
