...
void PatternTable::insert(uint64_t trigger, uint64_t second,  uint64_t third, uint64_t pc, uint64_t region_num, std::vector<bool> pattern,uint64_t current_cycle) {
    ...
    if (trigger != 0 || second != 1 || third != 2) { // not spatial streaming
...
PrefetchPrediction PatternTable::find(CACHE* cache, uint64_t trigger, uint64_t second,  uint64_t third, uint64_t pc, uint64_t region_num, int bw) {
    if (trigger != 0 || second != 1 || third !=64) { // not ss
...
// ------------------------- PB functions ------------------------- //
PrefetchBuffer::PrefetchBuffer(int size, int num_ways) :
    Super(size, num_ways) {}
...
void PrefetchBuffer::prefetch(CACHE* cache, uint64_t block_num, double prefetch_accuracy) {
...
STEP::STEP(int ft_size, int ft_ways, int at_size, int at_ways, 
           int pt_size, int pt_ways, int pb_size, int pb_ways) {
...
void STEP::access(CACHE* cache, uint64_t block_num, uint64_t pc) {
    ...
    if (at_entry) {
        ...
        if (!entry) {
            ...
            // Preserve the region for SOE/TOE training after the FOE lookup.
        ...
        // TOE
            ...
            auto at_victim = at->insert(region_num, entry->data.trigger_offset, entry->data.second_offset, region_offset, entry->data.pc, missed_in_pt, pattern_data_con);
            ft->erase(region_num);
            if (at_victim.valid) {
...
void STEP::prefetch(CACHE* cache, uint64_t block_num) {
    pb->prefetch(cache, block_num, prefetch_accuracy);
}
...
void STEP::update_accuracy_stats(CACHE* cache){
    if (cache->current_cycle - cycle_prev >= ACCURACY_DETECT_INTERVAL){
        ...
        prefetch_accuracy = accuracy_calculation(period_useless_prefetches, period_useful_prefetches);
...
double STEP::accuracy_calculation(uint64_t period_useless, uint64_t period_useful){
    double accuracy = 0;
    if (period_useless+period_useful!=0){
            ...
            if (period_useless+period_useful > 50){
                ...
                if (accuracy >= 0.75){
                    if (this->accuracy_streak < 4){
                        this->accuracy_streak += 1;
                    }
                }
                else {
                    if (this->accuracy_streak > 0){
                        this->accuracy_streak -= 1;
                    }
                }
            }
                ...
                if (accuracy >= 0.75){
                    ...
                    accuracy = 0.25*this->accuracy_streak;
                }
        ...
        if(this->zero_accuracy_intervals == 100){
            accuracy = 0.25;
            this->zero_accuracy_intervals = 0;
        }
        this->zero_accuracy_intervals += 1;
    }
    return accuracy;
}
