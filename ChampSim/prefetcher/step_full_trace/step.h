...
private:
    std::string last_two_offset_match_footprints = "-";
    void write_data(Entry& entry, custom_util::Table& table, int row);
...
public:
    ...
    const std::string& get_last_two_offset_match_footprints() const { return last_two_offset_match_footprints; }
...
private:
    ...
    bool trace_warmup_done_cycle_initialized = false;
    uint64_t trace_warmup_done_cycle = 0;
