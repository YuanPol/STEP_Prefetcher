...
uint32_t CACHE::find_victim(uint32_t triggering_cpu, uint64_t instr_id, uint32_t set, const BLOCK* current_set, uint64_t ip, uint64_t full_addr, uint32_t type)
{
  ...
  auto set_begin = std::next(std::begin(::rrpv_values[this]), set * NUM_WAY);
  auto [way_begin, way_end] = way_partition_bounds(type);
  auto begin = std::next(set_begin, static_cast<std::vector<int>::difference_type>(way_begin));
  auto end = std::next(set_begin, static_cast<std::vector<int>::difference_type>(way_end));
  ...
  assert(begin <= victim);
  return static_cast<uint32_t>(std::distance(set_begin, victim)); // cast pretected by prior assert
}
