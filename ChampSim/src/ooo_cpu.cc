...
constexpr uint64_t DEADLOCK_CYCLE = 10000000;

std::tuple<uint64_t, uint64_t, uint64_t> elapsed_time();
pattern_tracker::PatternTracker pt;
std::map<uint64_t, std::pair<uint64_t, uint64_t>> ooo_effect_tracker;
...
bool O3_CPU::do_fetch_instruction(std::deque<ooo_model_instr>::iterator begin, std::deque<ooo_model_instr>::iterator end) {
    ...
    // std::cout << fetch_packet.v_address << std::endl;
...
void O3_CPU::dispatch_instruction() {
    ...
    while (available_dispatch_bandwidth > 0 && !std::empty(DISPATCH_BUFFER) && DISPATCH_BUFFER.front().event_cycle < current_cycle && std::size(ROB) != ROB_SIZE
        ...
        auto instr = DISPATCH_BUFFER.front();
#ifdef TRACK_OOO_EFFECT
        for (auto smem : instr.source_memory) {
            if (ooo_effect_tracker.find(smem >> 12) == ooo_effect_tracker.end()) {
                ooo_effect_tracker.insert({smem >> 12, {((smem >> 6) & (64 - 1)), std::numeric_limits<uint64_t>::max()}});
            } else {
                if (ooo_effect_tracker.at(smem >> 12).second == std::numeric_limits<uint64_t>::max() && ooo_effect_tracker.at(smem >> 12).first != ((smem >> 6) & (64 - 1))) {
                    ooo_effect_tracker.at(smem >> 12).second = ((smem >> 6) & (64 - 1));
                }
            }
        }
#endif
...
#ifdef PATTERN_TRACK
    if (!warmup)
        pt.access(lq_entry.virtual_address, lq_entry.ip);
#endif
