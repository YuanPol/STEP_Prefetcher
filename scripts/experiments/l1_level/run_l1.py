#!/usr/bin/env python3
"""
Single-core L1 experiment launcher.
"""
...
def binary_path(prefetcher: str) -> Path:
    return REPO_ROOT / "ChampSim" / "bin" / f"champsim_1core_{prefetcher}"


def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run single-core L1 experiments.")
    parser.add_argument("--prefetchers", nargs="*", default=DEFAULT_PREFETCHERS, help="Prefetchers to evaluate.")
    parser.add_argument("--prefix", default="l1", help="Output prefix.")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    print(
        f"[l1] Workloads={count}, prefetchers={len(prefetchers)}, queued_runs={len(tasks)}, "
    ...
    if not tasks:
        print("[l1] Nothing to run.", file=sys.stderr, flush=True)
        return 0
