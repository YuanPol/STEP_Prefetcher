#!/usr/bin/env python3
"""
Multi-level (L1+L2) experiment launcher.
"""
...
L1_PREFETCHERS = {
    ...
    "enhanced_bingo": "bingo_streaming_disable_overlap_l1_l2_fill",
    "berti": "berti",
}
...
L2_PREFETCHERS = {
    "gaze": "gaze",
    "step": "step",
    "enhanced_bingo": "bingo_streaming_disable_overlap_pht_16x64",
    "berti": "berti",
}


def build_multi_level_prefetchers() -> List[str]:
    names: List[str] = []
    for l1_key, l1_name in L1_PREFETCHERS.items():
        for l2_key, l2_name in L2_PREFETCHERS.items():
            if l1_key == "berti" and l2_key == "berti":
                continue
...
DEFAULT_PREFETCHERS = build_multi_level_prefetchers()
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run multi-level prefetching experiments.")
    parser.add_argument("--prefetchers", nargs="*", default=DEFAULT_PREFETCHERS, help="Prefetchers to evaluate.")
    parser.add_argument("--prefix", default="ml", help="Output prefix.")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    print(
        f"[multi-level] Workloads={count}, prefetchers={len(prefetchers)}, queued_runs={len(tasks)}, "
    ...
    if not tasks:
        print("[multi-level] Nothing to run.", file=sys.stderr, flush=True)
        return 0
