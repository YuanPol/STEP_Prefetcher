#!/usr/bin/env python3
"""
Parameter sweep experiment launcher.
"""
...
# Legacy audit (json/1core_l2/withwarm_50000000_withsim_100000000, workloads_all):
# complete: all step_disable_second_ft*_at*_pt* variants below

def build_step_parameter_sweep(prefix: str) -> List[str]:
    return [
...
STEP_SWEEP = build_step_parameter_sweep("step_disable_second")

DEFAULT_PREFETCHERS = STEP_SWEEP
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--prefix", default="param", help="Output prefix.")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    print(
        f"[parameter-sweep] Workloads={count}, prefetchers={len(prefetchers)}, queued_runs={len(tasks)}, "
    ...
    if not tasks:
        print("[parameter-sweep] Nothing to run.", file=sys.stderr, flush=True)
        return 0
